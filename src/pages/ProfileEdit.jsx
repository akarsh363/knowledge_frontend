...
export default function ProfileEdit() {
  ...
  function getBackendBase() {
    ...
    return (env || fallback).replace(/\/+$/, '').replace(/\/api$/i, '')
  }

  async function fetchProfile() {
    try {
      const res = await api.get('/api/Users/me') // <- exact backend route
      setProfile(res.data)
      setFullName(res.data.fullName || '')
      setEmail(res.data.email || '')
      setDepartmentId(res.data.departmentId ?? '')
      // build preview from backend path if profilePicture exists
      if (res.data?.profilePicture) {
        const backendBase = getBackendBase()
        setPreview(`${backendBase}/${res.data.profilePicture}`)
      }
    } catch (err) {
      console.error(err)
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      // <- exact backend route for updating profile
      const res = await api.put('/api/Users/update-profile', fd, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })

      // Update local preview if backend returned new picture path
      if (res?.data?.profilePicture) {
        const backendBase = getBackendBase()
        setPreview(`${backendBase}/${res.data.profilePicture}`)
      }
            ...
            <div
              style={{
                ...
                backgroundImage: profile?.profilePicture
                  ? `url("${getBackendBase()}/${profile.profilePicture}")`
                  : 'none',
