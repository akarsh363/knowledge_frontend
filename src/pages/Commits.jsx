...
export default function Commits() {
  ...
  async function fetchCommits() {
    try {
      setLoading(true)
      // FIX: remove extra /api to avoid /api/api
      const response = await api.get('/Commits/mine')
      const data = response.data
      setCommits(Array.isArray(data) ? data : (data?.$values || []))
      console.log('Loaded commits from /Commits/mine')
    } catch (error) {
                  ...
                  <div className="flex justify-between items-start">
                    <div style={{ flex: 1 }}>
                      <div className="flex items-center gap-2" style={{ marginBottom: '0.5rem' }}>
                        ...
                        >
                          {/* status text intentionally left commented to preserve original UI choice */}
                        </span>
                      </div>
