...
export default function PostDetails() {
  ...
  async function handleDeletePost() {
    ...
    try {
      ...
      // Try to surface helpful server detail if present
      ...
      // show server message/detail if available
      const serverMsg = error?.response?.data?.message
      const serverDetail = error?.response?.data?.detail || error?.response?.data

      if (serverMsg || serverDetail) {
        // Keep the main generic text for UX consistency, but include server detail
        alert(`${serverMsg || 'Failed to delete post.'}\n\nServer detail (see console for full response):\n${serverDetail}`)
      } else {
